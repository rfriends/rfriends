...
### 1.3.5-beta5 (2026/03/14)         
  
1.3.4 -> 1.3.5-beta4
  
- ラジコプレミアムでログインできないことがあるのを修正。(beta5)  
  パスワードに英数以外が含まれるとエラーになっていた。  
  
- Windows版にvcruntime140.dllを同梱するようにしました。(beta4)  
  
- ~~ラジコプレミアムでセキュリティエラーでログインできないことがあるのを修正。~~(beta3)  
    
- ラジコ、らじるのリアルタイム録音の頭切れ対策を実施。(beta3)  
  理論上、頭切れは発生しないようになったはずですが、開始までの余裕が大きくなったかもしれません。  
  使用環境により開始遅れ時間が一定しないようなので一律に決めるのは困難。    
  
- timefree録音でセキュリティエラーで録音できないことがあるのを修正。(beta2)  
