...
> また、iSHでは、guiモードでも出てこないようです。    
>  [9-1-3] パラメータ設定 - チェック　で更新してください。  

## 1.6.3d (2026/09/25) 正式版  
...
・radiko podcast(d)  
　cueファイル  
  設定 - パラメータ - 編集　659行あたりで変更できます。   
  
・一部のチャプター不具合対応(c)  
  
・windowsでのチャプター不具合対応 (b)  
...
・一部でリスラジが聴取できないのを修正(d)  
  PHP8.5の仕様変更に対処。  

・delivery機能に、ftpを追加(d)  
  formatはsshと同様。     
  
・録音データ再生のバーを少し広げた(d)  
    
・管理 - 録音 データ - 削除にwebradio追加(c)  
    
・Windowsの録音ディレクトリ設定(sambaディレクトリ)を改善(b)  
  設定表示を追加し、チェック機能を設けた。  
    
・コミュニティFMにプリセット機能追加(a)  
　よく聞く局を登録できるようにした。  
  
## 1.6.2 (2026/08/06)  正式版  
  
1.6.1 -> 1.6.2     

・コミュニティFMに対応  
  リスラジ、JCBA に続いて、FM++に対応    
    
・radiko podcast のサイトの仕様変更に対応
  
・TIMERECのfile指定で、*指定をした場合に再生されないバグを修正  
  
・radiko,radiruのTIMEREC登録のバグ修正  
  
・webradioのTIMERECで、’（クオート）等の文字を使用した場合、  
 予約一覧が表示されなくなるバグを修正  
 例）job's  
    
 radiko等に合わせて文字変換します。’（クオート）は漢字のクオートになります  
  
・TIMERECで日をまたぐ指定のバグ修正  
  例）xxxx,SUN,23:30,00:30,yyyy,zzzz  
  
・らじるの局選択バグの修正   
  
・ラジコ、らじる、ウェブラジオのTIMEREC（時間指定予約）機能追加   
　曜日指定に  
　TODAY,TOMORROW,DAILY,MON-FRIを追加。    

## 1.6.1 (2026/07/14)  正式版  
  
1.6.0 -> 1.6.1   
  
・ラジコ、らじる、ウェブラジオの目覚まし機能にファイル再生追加。  
　radiko,radiru,webradioのtimerecの最後に,fileを追加するだけ。  

　r3,MON,16:50,15,基礎英語-レベル2*,english,file  
...
・JCBAの予約録音機能を追加  
 ...
 JCBAの聴取機能が使えない方  
https://rfriends.github.io/rfriends/tech/28.html
...
・webradio タグ、イメージ修正   
・webradio 予約補助 -> 予約に  
・webradio 転送（transfer）対応  
・timefree 聴取（サーバ）不具合対応   
...
- ListenRadioの聴取、聴取（サーバ）機能追加
- メール設定ツール追加
- OpenBSD,NetBSD対応
- ポッドキャストプリセットダウンロード機能修正
- task画面エラー修正  
...
- NHKポッドキャストをヘッドレスブラウザなしで動作するようにした。  
  しかも高速に。  
- DietPi対応  
- らじる聞き逃し(一括録音)が正常に動作してなかったのを修正  
...
- スマホ用フォントを変更。(1.4.2c)  
- 一部複数選択時に先頭選択から無選択に変更。(1.4.2c)  
- ポッドキャスト画面を修正。(1.4.2c)  
- 微修正。(1.4.2b)
- 表示を微修正(onair,削除)。(1.4.2a) 
- 表示を大幅修正しました。  
- NHK Podcastに対応。  
  JFN Podsに対応。   
  radiko Podcastに対応。 
  
- 設定にlightpandaのon/offスィッチを追加。  
...
- ish版のタスク表示を修正。 (1.4.1c)  
  
- ish版のエラー表示に対応。 (1.4.1b)    
  
**緊急リリースしました。(1.4.1)**　　
  
設定 - パラメータで  
数値のところに数値以外を設定すると  
rfriends3の右画面が真っ白になります。  
PHP8以降仕様が変わったためです。  
  
もし、そうなった場合は、  
ヘルプ - レスキューで1.4.1以降にアップデートしてください。  
   
### 1.4.0g (2026/04/09)  　          
  
1.3.5b -> 1.4.0g
  
- timerec登録機能を追加しました。 (1.4.0f,g)  
  放送局別の番組予約で「TREC登録」を行うと  
  予約の代わりにキーワードファイルに登録されます。    
  あとは、終了時間等を変更すれば、好きな時間の録音ができるようになります。  
  
- Windows版でエラーメッセージが出ていました。 (1.4.0e)  
  
- スマートホンでの表示を改善しました。 (1.4.0d)  
      
- 微修正。(1.4.0c)  
  名称を安定版から正式版に変更。   
  今後は、大きな変更がない場合は正式版の修正を行います。  
  
- 環境によりレスキューに失敗するのに対処。(1.4.0b)  
  
- 環境によりSSL証明書エラーが発生するのに対処。(1.4.0a)
...
- 予約関連のソースを整理しました。
- タスクビュー機能を追加。  
現在、実行している録音タスクを（ほぼ）リアルタイムで確認可能。  
- delivery機能にssh copy/move を追加。  
設定方法は、以下を参照。  
https://rfriends.github.io/rfriends/tech/25.html  
    
### 1.3.5b (2026/03/31)  
  
1.3.4 -> 1.3.5b    
  
- ゴガク一括録音が2026/03/30以降できなくなっていたのを修正。(1.3.5b)  
    
- ALSAデバイスの認識ができなくなっていたのを修正。(1.3.5a)  
  
- ラジコプレミアムでログインできないことがあるのを修正。  
  パスワードに英数以外が含まれるとエラーになっていた。  
  
- Windows版にvcruntime140.dllを同梱するようにしました。  
    
- ラジコ、らじるのリアルタイム録音の頭切れ対策を実施。  
  理論上、頭切れは発生しないようになったはずですが、開始までの余裕が大きくなったかもしれません。  
  使用環境により開始遅れ時間が一定しないようなので一律に決めるのは困難。    
  
- timefree録音でセキュリティエラーで録音できないことがあるのを修正。  
  
- radiru予約一覧表示を修正しました。  
  表示異常のみで実害はありません。
  
- **macOS で定期実行ができない不具合に対処しました。**  
  update後、再登録してください。  
  
- **macOS で予約録音ができない不具合に対処しました。**  
  
### 1.3.4 (2026/02/27)         
  
1.3.3a -> 1.3.4 
  
- **同一時間開始の録音が複数ある場合、まれにusrdirが省略時の場所に変わってしまうのを修正しました。**    
  省略時の場所の例：~/rfriends3/usr
  
- 配信遅れデータの改定を行いました。  
　配信遅れデータを修正している方は、updateの前にkw/delay.datをバックアップしてください。  
    
- ffmpeg8.0でのエリアフリー録音を見直しました。    
- at,cron-> systemdに関する修正を行いました。  
　現状、archlinux系以外は変更なし。  
  
### 1.3.3a (2026/02/23)         
  
1.3.2 -> 1.3.3a  
  
- timerecで’FM_OITA’と’FM_OKINAWA’がエラーになるのを修正しました。  
