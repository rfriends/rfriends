...
### 079 チャプターありとなっているのにチャプターが表示されません。  
  
一部のニュース番組等で、チャプターが一つのものがあります。  
この場合、実質チャプターなしになります。  
  
 例）JORF_ラジオ日本ニュース・天気予報  
 ```
  Chapters:
    Chapter #0:0: start 0.000000, end 593.720000
      Metadata:
        title           : ニュース・天気予報：米中首脳会談から台風被害、関東の雨予報まで
```
  
### 078 VLCで再生したとき、違うアートワークが表示されます。  
  
vlcが違うアートワークをキャッシュで保持しているためです。  
他のプレーヤで正しく表示されるか試してください。  
  
安全なやり方は、vlcの再インストールです。  
アンインストール時にキャッシュをクリアするを選択してください。  
  
### 077 メール設定をしたのですが、エラーになります。  
あるいは、最近、メールが届かなくなりました。    
  
yahoo側でいつのまにか許可しないに設定したようです。  
yahooメールのページを開いて、歯車アイコンをクリックし、  
SMTP,POPを利用するにして、保存して下さい。  
その後、テストメールを実行して、メールが来ることを確認してください。  
  
### 076 WebRadioを聴こうとしているんですが、音が途切れます。 
  
他の局も同様ですか?  
その局にかぎって、音が途切れるのであれば、それはその局の設備の問題です。   
他の局を聴いてみてください。  
  
### 075 WebRadioを聴こうとしているんですが、音が出ません。 
  
真夜中に聴いていませんか?  
局によっては24H放送ではなく、夜中は放送を停止しています。  
その際は、無音放送になります。  
他の局を聴いてみてください。  
     
### 074 JCBAの聴取（サーバ）はできるけれど、聴取ができない。    
  
WEB サーバーがlighttpd の場合、lighttpd.confの末尾に以下を付け加え再起動してください。  
...
,play
を追加してください。  
radiko, radiru, webradioで有効です。(1.5.0c以降)    
  
### 072 Linux版の聴取（サーバ）で音が出ない。  
  
実行ユーザのグループにaudioを追加してみてください。  
...
### 067 同時予約数が制限を超えているため録音予約できません  
  
radiko、radiruの予約で、上記メッセージが出て予約できません。  
  
標準では同一時間帯４までとなっています。  
この数を大きくすれば予約できるようになります。(reserve_limit)  
  
ただし、非力なマシンで大きくしすぎると録音失敗の確率が上がります。  
...
### 066 録音するファイルのビットレートが 48kbps になっているのですが  
  
ビットレートをあげて高音質にできませんか？  

現在、ラジコが48K、らじるが96Kで配信されており、受信したものを加工せずにそのまま保存しております。  
デジタルデータですので、ビットレートをあげても劣化するだけでメリットはありません。  
  
### 065 sambaにアクセスできない。  
  
Windows11を24H2/25H2更新後、sambaアクセスを行うと  
「組織のセキュリティポリシーによって非認証のゲストアクセスがブロックされているため  
この共有フォルダーにアクセスできません」と表示される場合があります。  
...
詳細は、以下が参考になります。  
[大塚商会](https://faq.mypage.otsuka-shokai.co.jp/app/answers/detail/a_id/320005/~/%5Bwindows11%5D-24h2%E6%9B%B4%E6%96%B0%E5%BE%8C%E3%80%8C%E7%B5%84%E7%B9%94%E3%81%AE%E3%82%BB%E3%82%AD%E3%83%A5%E3%83%AA%E3%83%86%E3%82%A3%E3%83%9D%E3%83%AA%E3%82%B7%E3%83%BC%E3%81%AB%E3%82%88%E3%81%A3%E3%81%A6%E9%9D%9E%E8%AA%8D%E8%A8%BC%E3%81%AE%E3%82%B2%E3%82%B9%E3%83%88-%E3%82%A2%E3%82%AF%E3%82%BB%E3%82%B9%E3%81%8C%E3%83%96%E3%83%AD%E3%83%83%E3%82%AF%E3%81%95%E3%82%8C%E3%81%A6%E3%81%84%E3%82%8B%E3%80%8D%E3%81%A8%E8%A1%A8%E7%A4%BA%E3%81%95%E3%82%8C%E5%85%B1%E6%9C%89%E3%83%95%E3%82%A9%E3%83%AB%E3%83%80%E3%81%AB%E3%82%A2%E3%82%AF%E3%82%BB%E3%82%B9%E3%81%A7%E3%81%8D%E3%81%AA%E3%81%84)  
  

### 064 ブラウザでrfriends3にアクセスするとファイルがダウンロードされる。  
  
もし、Chromeで発生しているなら、edgeで試してみてください。（またはその逆）  
別のブラウザでは正常なら、ブラウザの閲覧履歴、キャッシュをクリアしてみてください。  
  
### 063 radiko/radiruのリアルタイム録音で、開始が遅いまたは最後が切れているんですが？
  
ストリームの遅れ調整が必要です。  
以下の調整を行ってみてください。  
  
radikoの遅延パラメータの調整方法  
https://rfriends.github.io/rfriends/tech/20.html  
