...
~~```trixie系の場合、lighttpd版では予約ができないようです。（問題ないという報告もあり）```~~  
~~Apache2版を追加しました。(2025/08/15)~~  
現在、apache2版は公開を中止中。(2026/04/24)    
...
|2026/04/24|〇| Debian (Trixie)|13.4|~~apache版使用~~|  
...
> [!CAUTION]  
> ただし、ubuntu 26.04 および  debian13 の場合、以下の操作をしないと録音予約ができません。  
> $ cd ~/  
> $ cd rfriends3_core  
